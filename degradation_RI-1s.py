...
expName = 'degradation'  # from the Builder filename that created this script
expInfo = {'version': 'A', 'participant': 'DEG'}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/rich/Documents/Github/hunger-games/degradation_RI-1s.py',
...
Instruc_text = visual.TextStim(win=win, name='Instruc_text',
    text='The vending machines are malfunctioning and one of the snacks is falling out without tilting! \n \nFind out which direction causes snacks to drop. ',
    font='Arial',
...
if expInfo['version'] == 'A':
   ...
   msgA = 'Did tilting the machine cause an M&M to drop?'
   msgB = 'Did tilting the machine cause a BBQ shape to drop?'
   logging.log(level=logging.DATA, msg = 'version A')
if expInfo['version'] == 'B':
   rewardA = 'images/BBQ.png'
   rewardB = 'images/M&M.png'
   msgB = 'Did tilting the machine cause an M&M to drop?'
   msgA = 'Did tilting the machine cause a BBQ shape to drop?'
   logging.log(level=logging.DATA, msg = 'version B')
if expInfo['version'] == 'C':
   rewardA = 'images/teddy.png'
   rewardB = 'images/BBQ.png'
   msgA = 'Did tilting the machine cause a tiny teddy to drop?'
   msgB = 'Did tilting the machine cause a BBQ shape to drop?'
   logging.log(level=logging.DATA, msg = 'version C')
if expInfo['version'] == 'D':
   rewardA = 'images/BBQ.png'
   rewardB = 'images/teddy.png'
   msgB = 'Did tilting the machine cause a tiny teddy to drop?'
   msgA = 'Did tilting the machine cause a BBQ shape to drop?'
...
conditions = data.TrialHandler(nReps=1, method='sequential', 
    ...
    trialList=data.importConditions('Conditions-DegRI-1s.xlsx'),
...
for thisCondition in conditions:
        ...
        while continueRoutine:
            ...
            if timer2.getTime()<0 and PressB == True:
             ...
             if RR == 1:
              ...
              nRepB=1
              continueRoutine=False
            if timer2.getTime()<0 and PressA == False and PressB == False:
             timer2.reset(1)
             A = randint(lowerA,upperA)
             logging.log(level=logging.DATA, msg = A)
             B = randint(lowerB,upperB)
             #logging.log(level=logging.DATA, msg = B)
             if A == 1:
              logging.log(level=logging.DATA, msg = 'freeA')
              nRepA=1
              nRepB=0
              continueRoutine=False
             if B == 1:
              logging.log(level=logging.DATA, msg = 'freeB')
              nRepA=0
              nRepB=1
              continueRoutine=False
    ...
    while continueRoutine:
        ...
        if testSnackA.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > testSnackA.tStartRefresh + 10-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if rateA_text.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > rateA_text.tStartRefresh + 10-frameTolerance:
                # keep track of stop time/frame for later
    ...
    conditions.addData('sliderA.rt', sliderA.getRT())
    ...
    while continueRoutine:
        ...
        if testSnackB.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > testSnackB.tStartRefresh + 10.0-frameTolerance:
                # keep track of stop time/frame for later
